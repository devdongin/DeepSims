...
규칙: (0) **지표를 누르는 처방 금지** — 오류 수치(no_path·lonely·불만·starving)를 낮추려 파라미터를 튜닝하거나 실패를 억제하는 제안은 하지 마라. 대신 **사람이 할 법한 행동을 하나 더 추가**해 세계가 스스로 풀게 하고, 그렇게 풀리는지 관찰할 지표를 함께 제시하라. 단 계약 위반(결정성·오버플로·리플레이 불일치)과 명백한 구현 결함은 그대로 고친다. (1) 인구를 직접 조작하는 제안 금지 — 창발 경로만. (2) 기존 §17/§18 계약(결정적 tick, 내구 입력, 드로우 순서)을 깨지 않을 것. (3) 이미 열린 이슈와 중복되면 '기존 #N 확장'으로 표기. (4) 가능하면 실제 논문·게임(Anno1800/문명/시티즈) 사례를 근거로 인용.
출력은 마크다운 목록만." </dev/null > "$OUT_DIR/roadmap-$STAMP.md" 2>&1 || echo "codex 실패" >> "$OUT_DIR/roadmap-$STAMP.md"
