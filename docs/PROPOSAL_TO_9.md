# 제안: 평가 평균 5.5 → 9.0 (Opus 구현용 로드맵) — v6, Codex 5차 NO-GO 조건 반영
...
- 완료: 입력 10건·공란·구세이브·악의적 `surname` 주입 각 10건에서 피드·패널·이름표 표시 전부 동일, 주입은 서버가 제거, 미등록 토큰 0. **오라클 고정**: 임시 표기(스냅샷 조회)와 C-2 이후 표기(`payload.names`)의 문자열이 같은 fixture에서 100% 일치.
...
- 완료: 배속 ×48·인구 100에서 30분간 전면 재빌드 0회, Text 생성 수 ≤ 변경 심 수, resync 페이로드 ≤ 200KB, p95 프레임 < 16.7ms.
...
- 완료: 8/8 렌더, 각 노드에 현재값·목표값·상태·도달 행동 표시; **성공 fixture**(인구를 25로 성장시킨 세계에서 노드 1 done) 5/5,
  **실패 fixture**(인구 정체 세계에서 노드 1 미달성·힌트 표시) 5/5; 달성 판정은 체크박스가 아니라 **실제 전이 이벤트**로.
...
**T3-2 조명** — `applyDaylight(tick)` 룩업. 완료: 05:00/07:00/12:00/19:00 네 tick에서 **고정 probe 픽셀 3점(좌표: 하늘 (20,20)·건물 창 (560,420)·바닥 (300,700), 1120×848 캡처 기준)**의 인접 시각 간 색차 **ΔE ≥ 5**, 정오 점등 0, stale 상태 0.
