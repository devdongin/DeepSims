...
- **정직한 한계 — 이 슬라이스는 세계의 고통을 줄이지 못한다.**
  `no_money` 404→375, 굶주림 91→83, 인구 59→65로 움직였지만 **이건 효과의 증거가 아니다.**
  실제 거동 차이는 바리스타 임금 부족 2,120원(30일 총합)뿐인 극소 섭동이고, 나머지는
  결정적 시뮬레이션이 그 섭동에서 갈라진 **궤적 차이**다. 인과를 주장하지 않는다.
  증명된 것은 하나다: **사라지던 922,680원이 이제 장부에 남아 어디에 고이는지 보인다.**
