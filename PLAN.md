...
- 보정치 클램프는 전형 점수(욕구 5000, 거리 14 기준 base ≈ 1.3e12)의 ±20~40% 수준으로 설계 —
  보정이 의미는 갖되 욕구 항을 지배하지 못함.
- 캡: 습관 누적 상한(행동당 habitMod ≤ 2.5e11, 증가분/일 ≤ 1e10), 계획 보너스 ≤ ×1.5,
  성격 [50,200]. 장기 실행에도 욕구 항 비지배(테스트 22로 강제).
...
20. 수면 중 직렬화(pendingMood 보존) / 21. 계획 하루 1회 생성 / 22. 습관 캡·장기 점수 바운드(욕구 항
비지배) / 23. 확산(토큰 순서·rng 소비 고정) / 24. 파티 참석 재현성(같은 시드 → 같은 참석자) /
