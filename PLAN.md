...
- **'오는 중'인 심도 센다 (walkingPct 50%)** — 예상과 다른 역할을 했다. 헛걸음을 줄이는 게 아니라
  **쏠림 폭주를 막는 감쇠 신호**였다. walkingPct=0이면 헛걸음은 27.2%로 더 낮지만
  **no_path가 1,973건** 터진다. 모두가 같은 '현재 인원'만 보고 한 곳으로 몰리기 때문이다.
- **A/B 소크 (같은 라이브 스냅샷 Day145 → 14일)**:
  ...
  | 장소에서 혼자인 비율 | 80% | **61%** |
  | no_path | 0 | **0** |
- **강도 절벽**: `gravityPullPct`를 150까지 올리면 헛걸음은 18.9%까지 내려가지만
  no_path가 1,880건 터진다. 60은 그 절벽에서 충분히 떨어져 있다.
