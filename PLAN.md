...
## #88 경찰·소방 관측 (별도 원장, 구현 검증 중)

실제 가상 목표 선택을 공유하되 후보·경로·RNG는 보존한다. 시도/시작/행동 완료/
경로 실패와 일 단위 사전 막힘을 구분하며 14일만 저장한다. 실제 진압은 소방관의
수행 상태와 fire_out을 대조하고 자연 소화/늦은 완료와 구별한다. 경찰 교대 시간과
기존 비번 소방 대응 규칙을 그대로 따른다. 관할별 요약만 HTTP/snapshot/batch 및
지표 대시보드에 공개하고 주민 중복 제거 ID는 저장 안에만 둔다. #154 정원을
변경하거나 기존 산업 원장의 수요를 위조하지 않는다. 상세·리뷰 응답은
docs/public-service-observation.md에 기록한다.
...
## #32 사용자 중심시 교차 도로 부지 복구 (logic109)

원본 보관본 readonly 조사에서 center0/plot73(186,120)이 도로x192/y115와 인도y125에
막혀 있었다. 큰 대학12×10을 보호하는 복구 탐색 반경8로는 거리12의 유효 부지
(180,126)에 도달하지 못한다. 반경을 보호 footprint의 폭+높이로 유도하고 같은 관할
중심시 영향권 안으로 제한한다. 타일을 지우지 않고 기존 필지·건물·예약을 보존한다.
유상 대형 공항 예약은 일반12폭 필지가 아니라 실제20×12 footprint도 검사한다.
실제 보관본 메모리 복사에서12×10 건축 가능 및 전체 타일 불변을 확인했고,
원본 지형을 축소한 회귀는 결정적 재개와 정상 자동 착공 경로까지 검증한다.
현재 플레이 파일은 다른 시드/중심시 입력 없음. 원래 세계 복구나 기존 대학 확대를
자동 수행하지 않는다. docs/center-campus-relocation.md에 근거와 한계를 기록한다.

## §23.56 결과 불변 최적화 둘 — stateModFor의 pop² 항(#99), isWalkable 표(#98 ②)

3회차 성능 리뷰가 CPU 프로파일로 지목한 자리다. 둘 다 **결과 불변**이 계약이다(#99 본문:
"행동을 바꾸는 튜닝 금지 — 결과 불변 최적화만, 결과 해시 선행").

**#99 `stateModFor`** — 후보 시설마다 불리는데 그때마다 **전 인구를 훑었다**.
인구 200·시설 112·결정 1,875건/일이면 하루 4,240만 회 순회, 프로파일에서 틱의 12.8%,
그리고 인구 100 → 200 → 400에서 µs/sim-tick이 22.6 → 49.2 → 93.9로 두 배씩 뛰던
pop² 항의 원인이었다. 결정 한 번에 `performingIndex(world)`(시설 → performing 중인 심,
world.sims 순서)를 **한 번** 만들어 넘긴다. 틱 단위 스냅샷으로 만들면 앞 순번이 방금 앉은
것을 뒷 순번이 못 보게 되어 의미가 바뀌므로(§23.10과 반대 방향의 사고), **심 단위**로만
만든다 — 한 심의 결정 안에서는 아무 상태도 안 바뀌므로 첫 스캔 값과 정확히 같다.

**#98 ②** — BFS 안쪽 루프의 `isWalkable`이 방문 칸마다 `Set.has`를 불렀다. 타일 종류별
`Uint8Array` 표로 바꾸고, 표에 없는 값은 옛 술어로 떨어져 같은 답을 낸다.
(#98 ①의 세대 스탬프는 43199f0에서 이미 들어가 있었다.)

### 증명
- 3시드 × 30일 `hashWorld`·이벤트 수·인구가 HEAD와 **전부 동일**(1000/8919/424242).
- #97 골든 벡터(후보·선택·이벤트·세계) 세 인구 모두 **변경 없음**.
- 벤치의 결정/일·BFS 호출/일·방문칸/호출이 1,891 / 1,899 / 6,174로 동일 — 같은 일을 했다.

### 측정 (popscale 인구 200·2일, HEAD↔NEW 교차 3라운드, 기계 경합 중)
| 라운드 | HEAD tick/s | NEW tick/s | HEAD µs/sim-tick | NEW | BFS 비중 |
|---|---|---|---|---|---|
| 1 | 53 | 66 | 93.96 | 75.18 | 15% → 12% |
| 2 | 54 | 89 | 92.91 | 55.97 | 15% → 11% |
| 3 | 124 | 166 | 40.03 | 29.89 | 15% → 12% |

절대값은 경합 때문에 라운드마다 2배 넘게 흔들린다 — 그래서 교차로 돌리고 방향만 읽는다.
**3/3 라운드 NEW 우세**, 최선값 기준 +34%(124 → 166 tick/s), ms/일 11,587 → 8,652.

## §23.57 공공 식사의 지역 조달 (#164, logic107)

공공 식사의 재료비는 전액 externalOutflow(마을 밖 조달)였다. G1 회계는 닫혀 있었지만
굶주린 사람이 공공 식사를 먹어도 시장·농장 매출이 0이었다 — 마을이 자기 식량을 기르는데
(grow/supply_groceries, 계절 비축) 복지는 수입 식량으로 돌아갔고, 세율·복지를 움직여도
산업 지표에 닿을 통로가 없었다. 이슈의 선택지 1(지역 조달)을 택했다.

규칙: 같은 마을에서 재고 ≥ 1이고 사고가 없는 식품 판매 시설 중 **가장 작은 id**(코드가
정렬을 보장, food-supply 발주 순회와 같은 compare)에서 1단위를 단가로 산다. 국고는 종전대로
mealCost 전액을 내고, 그중 min(mealCost, unitPrice)은 그 가게 매출로, 나머지는 외부로.
재고가 없을 때만 전액 외부 조달. 복지 조달분은 `foodSupply.totals.publicMealUnits`로 따로
센다(소매 soldUnits와 섞지 않는다). rngSim 미소비.

Codex 합의: 0(조건부) → 세 조건(회귀 테스트 갱신·id 정렬 계약 명시·soldUnits 의미 확정)
전부 반영. G1 회계·다중 마을 귀속·RNG 비소비는 동의.
실측(시드 1000, 30일): 공공 식사 33건 중 32건 지역 조달, 폐쇄 회계 잔차 0.
골든 벡터: 세계 해시만 이동(240틱 안에 공공 식사 없음).

## §23.59 화재 지점 고착 — 갈 수 없는 불에는 달려가지 않는다

#164를 넣자 철도 70일 픽스처가 `noPath == {}` 단언에서 깨졌다: village:0에 no_path 642건.
프로브(관측자에 화재 스팟 좌표·통행·지역을 기록하도록 계측)로 보니 **50/50 전부
`respond_fire → firesite:fire:school`** 이었고, 실패 시점에 그 스팟(문 북쪽 한 칸)은
walkable=false·sameRegion=false, 문 자체는 walkable=true였다 — 나중에 그 칸에 무언가
지어진 것이다. 같은 응답자가 **매 틱** 같은 불을 다시 골랐다(127344, 127345, 127346…).

원인은 tick.js의 화재 가상 스팟이 `door.y − 1` 한 칸으로 못박혀 있던 것이다. 순찰은
§19.4(이슈 #40)에서 똑같은 고착을 이미 고쳤는데 화재는 그대로였다. #164 자체의 버그가
아니라 궤적이 바뀌며 드러난 잠재 결함이다 — 60일 새 마을 리셋 이후 라이브에서도 언제든
같은 일이 날 수 있었다.

수정: 순찰과 같은 규칙 — 문 주변 북→남→동→서→문 순서에서 **걸을 수 있고 응답자와 같은
지역인** 첫 칸을 스팟으로 고르고, 없으면 그 불은 이 응답자의 후보가 아니다.
쿨다운 필터는 넣지 않았다 — 상류의 명시 계약("historical lack of cooldown filtering",
불은 매 틱 다시 볼 일이다)과 충돌했고, 고착의 원인은 재선택이 아니라 도달 불가 스팟이라
그것만 막으면 충분하다. rng 미소비, 결정적. 회귀 테스트 S-89(평소 스팟·봉쇄된 문).

교훈: **가상 스팟(순찰·화재·공사장)은 전부 같은 도달 가능성 규칙을 써야 한다.**
하나만 고치면 나머지가 다음 궤적 변화에서 같은 모양으로 깨진다.
