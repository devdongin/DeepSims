...
- **시간**: 1틱 = 게임 속 1분. ×1배속에서 **게임 속 하루 = 실제 10분**, **1년 = 40일**입니다.
  배속은 ×1 / ×4 / ×10 / ×20 (×20이면 하루가 30초). 페이싱 근거는 §23.44 — 스타듀밸리의
  하루가 실제 14분인데 그 14분 내내 플레이어가 무언가를 하고, DeepSims는 보는 게임이라
  같은 시간이 더 길게 느껴집니다. 1년을 120일에서 40일로 줄인 뒤 100일당 생애사건
  (졸업·결혼·출생·은퇴·사망·성장)이 26.2 → 41.9로 늘었습니다.
...
**클론하면 당신만의 마을이 처음부터 시작합니다.** 세계의 상태(`deepsims.db`)는 저장소에
포함되지 않습니다 — `.gitignore`가 `*.db`·`*.db-shm`·`*.db-wal`을 막고 있고, 저장소 이력에도
DB가 커밋된 적이 없습니다. 첫 `npm start`에서 **Day 0 · 인구 10 · 국고 2,000원**으로 새 세계가 생성됩니다.
- `logs/`도 추적하지 않습니다. 예외는 `logs/world-snapshot.json` 하나인데, 이건 **개발자 저장소의
  세계 상태를 GitHub Actions·리뷰 에이전트에 넘기기 위한 리포트 파일**입니다.
  게임은 이 파일을 읽지 않으므로 당신의 세계에 아무 영향이 없습니다(지워도 됩니다).
**설계 근거**: 이 세계의 행동 규칙은 사회·경제 실증 연구에 기대고 있습니다 —
출처와 라이선스, 어느 로직에 쓰였는지는 [`docs/REFERENCES.md`](docs/REFERENCES.md)에 있습니다.
(논문 전문은 저작권 때문에 저장소에 두지 않고 링크로만 참조합니다.)

**시드**: 처음 실행할 때 **당신만의 시드가 무작위로 뽑힙니다** — 지형도, 주민도, 이름 순서도
사람마다 다릅니다. 서버가 시작하면서 그 시드를 알려주니 적어 두세요.
- 같은 마을을 다시 만들거나 친구와 나누려면: `DEEPSIMS_SEED=<시드> npm start` (DB를 지운 뒤)
- 한 번 만들어진 마을은 시드 환경변수와 무관하게 그대로 이어집니다
...
- [x] 사회 시뮬레이션 (§17): **512×512** 맵, **이민**(주거가 늘면 새 주민이 온다),
      ...
      **험담의 사회적 영향**(말이 청자의 관계 인식을 실제로 바꾼다), 결혼식 마을 잔치,
      선거 유세(인기인이 거리로 나선다), 새해·졸업·은퇴의 생애 주기, **가족과 자녀**(동거하는
      부부에게 새해에 아이가 정착한다 — "우리 애가 벌써 학교엘 다녀"), 별거 부부의 신혼집 수요가
      건설을 부르고, 집이 지어지면 합가하고, 아이가 태어나면 침대를 늘리는 **성장 루프**,
      식당·헬스장·영화관(비 오는 날의 실내 피난처), 30일마다 열리는 마을 축제 🏮
- [x] 생활 리듬 (§17.13/16/17): **크로노타입**(아침형·저녁형이 출퇴근을 바꾼다), J형은 야근하고
      P형은 칼퇴하는 **가중 확률 성격**, 경찰·소방관·간호사의 **주야 교대**(새벽에 일하는 사람),
      **서카디언 수면 압력**(밤에 졸리다, 야간조는 낮에), **주중/주말**(주말엔 공원이 붐빈다)
- [x] 경제 순환 (§17.15): **소득세 → 국고 → 복지**(가난하면 정부가 지원)·시장 수당, 상단바 국고 실시간
- [x] 도시 성장 드라이브 (§17.21): 결혼·축제·완공이 **마을 평판**을 쌓고, 평판이 **이민 웨이브**를
      부르고, 선제 주택·일자리(사무소) 건설이 뒤따른다 — 90일에 인구 10→69의 성장 폐루프
- [x] 사건 (§17.20): **화재** 🔥 — 소방관이 달려와 진압하면 영웅이 되고, 소방력 없는 마을은
      불이 방치돼 평판이 깎인다
- [x] **타이쿤 레이어** (§18): 플레이어=시장 — 🏛️ 세율·복지 **시정 슬라이더**(납세 불만 트레이드오프),
      📐 공터 클릭 **건설 지시**(용도+4방향 회전, 국고 차감), 도시 등급 **마을→읍→시→대도시**
      (승격 폭죽·시설 언락: 아파트·공장·상가·대학), 📊 지표 대시보드(인구·국고·평판 180일)
- [x] 치안·정직 (§17.24): 경찰 **순찰**(다닐수록 평판 유지 — 공장 공해의 대칭), 동전을 주우면
      성격 따라 파출소에 **분실물 신고**, 3일 뒤 정직한 발견자의 몫
- [x] 비주얼 (§17.14/19): 치비 도트 캐릭터 **4방향 보행·서기·앉기**, 직업 **제복**(경찰복·소방복·
      가운), **목업풍 건물 16종**과 클릭하면 열리는 **내부 컷어웨이**, 야간 점등, 경찰차·소방차·구급차, **지면 텍스처**(잔디 패턴·도로 아스팔트)
