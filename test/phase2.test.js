...
test('P2-9. v1 스냅샷 마이그레이션: traits/mood/logic 설치, 결정적', () => {
  ...
  assert.equal(m1.schemaVersion, 5);
  assert.ok(m1.logic);
...
test('P2-14. 마이그레이션 시 meta 영속화 (schemaVersion·behaviorVersion)', () => {
  ...
  assert.equal(loaded.world.schemaVersion, 5);
  assert.equal(st.getMetaInt('schemaVersion'), 5);
  assert.equal(st.getMetaInt('behaviorVersion'), 5);
