// 합성 인구 생성 유틸 (이슈 #17) — **고정 인구 부하** 벤치용 세계를 결정적으로 만든다.
//
...
//
// 대표성 한계 (Codex 교차 리뷰 ③ — 해석할 때 반드시 기억할 것):
// 이 세계는 "인구 P의 성장 월드"의 대표값이 아니라 **인구 P 고정 조건의 결정·경로
// 부하 fixture**다. 실제 성장 월드와 다른 점:
//  - 주거가 전부 아파트 (실제 플래너는 house 위주) — 시설 수·문 위치 분포가 다르다
//  - addBuilding 직접 호출이라 건설비·국고·건설 노동 이력이 없다
//  - 전원 tick 0 일괄 생성이라 기억·관계·재정·나이 분포가 초기값이다 (워밍업 1일로
//    일부만 채워진다 — 기억 만재 비용은 bench.js 최악 케이스가 따로 잰다)
//  - 격자 스캔 시설은 도로 인접이 아닐 수 있다 (초지 연결성만 보장)
// 그래서 popscale 수치는 "같은 fixture에서의 전후 비교"와 "스케일 곡선의 모양"에 쓰고,
// 살아 있는 세계의 절대 비용 예측에는 쓰지 않는다.
