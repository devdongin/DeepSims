...
let studied = 0, paidStudentWork = 0, hungryPersonTicks = 0, personTicks = 0;
...
for (let i = 0; i < Number(daysArg) * 1440; i++) {
  for (const s of w.sims) {
    if (s.state.kind === 'performing') {
      ...
      if (s.state.action === 'work' && s.traits.occupation === 'student') paidStudentWork++;
    }
  ...
  for (const e of tick(w)) counts[e.type] = (counts[e.type] ?? 0) + 1;
}
console.log(JSON.stringify({ inputSha256:createHash('sha256').update(raw).digest('hex'),
  ...
  paidStudentWorkPersonTicks:paidStudentWork,
